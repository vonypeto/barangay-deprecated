<header class="header-blue" style="padding-bottom: 0px;">
    <nav class="navbar navbar-dark navbar-expand-md navigation-clean-search">
        <div class="container-fluid"><a class="navbar-brand" href="/barangay/home" style="font-size: 45px;font-family: bodoni mt;"> <img class="logo" src="{{  url($image->url ?? 'Logo not set')  }}" style="resize: both;width: 80px;margin-right: 30px;"><p class="navbar-brand" id="client_navbar" style="font-size: 45px;">{{ $image->barangay_name ?? 'Barangay not set'  }}</p></a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
...
<ul class="nav nav-tabs" style="margin-left: 20px;">
    ...
    <li class="nav-item dropdown"><a class="dropdown-toggle nav-link {{ (request()->is('barangay/schedule*')) ? 'active' : '' }} {{ (request()->is('barangay/blotter*')) ? 'active' : '' }} {{ (request()->is('barangay/certificate*')) ? 'active' : '' }}" aria-expanded="false" data-toggle="dropdown" href="#"><i class="fa fa-server" style="margin-right: 5px;"></i>Services</a>
